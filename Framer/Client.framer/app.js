...
(function() {
  // --- Setup
  var ViewController, Views, b_Splash, but_Begin, f, heading, i, label, len, logo, rectangle, ref, splash_Body, test, title, x, ƒ, ƒƒ;
  ...
  for (i = 0, len = ref.length; i < len; i++) {
    f = ref[i];
    f.on(Events.TouchStart, function(event, layer) {
      return layer.ƒ("rec*").animate({
        image: Utils.randomImage(),
        backgroundColor: Utils.randomColor()
      });
    });
    f.on(Events.TouchEnd, function(event, layer) {
      return layer.ƒ("rec*").animate({
