...
updateTxtValues = ->
  for key,val of currentData

    if (val is "") or(key is "Matters") then continue
    ƒ("#{key}_in").value = val
...
for v,k of ppl
  ...
  for k1,v1 of k
    ...
    @["#{k1}"] = new TextLayer
      ...
      backgroundColor: "transparent"
    l++
  contentsEntries[v].on Events.Tap,  (event,layer) ->
    flow.showNext edit, scroll:false
    print @custom.indi
    for k2,v2 of ppl[@custom.indi]
      if k2 isnt "lang" then currentData[k2] = v2
    updateTxtValues()
mattersPanel = new Layer
  parent: p2
  x: Align.right(-80)
  y: Align.center
  width: (p2.width/2)-80
  height: p2.height - 160
  borderRadius: p2.borderRadius
  backgroundColor: "transparent"
  borderWidth: 6
  borderColor: silver
matterPickOverlay = new Layer
  width: Screen.width - 500
  height: Screen.height - 500
  point: Align.center
  backgroundColor: "transparent"
addMatter = new MyButton
  parent: mattersPanel
  backgroundColor: "transparent"
  x: 20
  y: 30
  ...
  height: 80
  txt: "New Matter"
  bCol: cameraInput.backgroundColor
  dCol: cameraInput.borderColor
  tCol: cameraInput.style.color
addMatter.onTap ->
  flow.showOverlayCenter matterPickOverlay
matterLayers = []
currentData.Matters.push
  Type: "Theft"
  Outcome: ""
  Fact_Sheet_Number: "07867"
  Adjouned_Date: ""
drawMatterLayers = ->
  matterLayers.length = 0
  for matter, i in currentData.Matters
    matterLayers[i] = new Layer
      parent:mattersPanel
      borderWidth:4
      width:mattersPanel.width
      height:150
      x:-6
      y:144 + (150*i)
      backgroundColor:"transparent"
      borderColor: silver
    vcount = 0
    for tkey, tval of matter
      print tkey + ":" + tval
      @["tkey"] = new TextLayer
        parent: matterLayers[i]
        width: mattersPanel.width / 4
        x:vcount * (mattersPanel.width / 4)
        y: 32
        html: "#{tkey.replace /_/g, " "}<br><b>#{tval}</b>"
        style:
          textAlign: "center"
          fontSize: "32px"
